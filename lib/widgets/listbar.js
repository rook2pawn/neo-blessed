...
Listbar.prototype.setItems = function(commands) {
  ...
  if (!Array.isArray(commands)) {
    commands = Object.keys(commands).reduce(function(obj, key, i) {
      ...
      if (cmd.prefix == null) cmd.prefix = String(++i);
...
Listbar.prototype.appendItem = function(item, callback) {
  ...
  if (typeof item === 'object') {
    cmd = item;
    if (cmd.prefix == null) cmd.prefix = String(this.items.length + 1);
  }

  if (typeof item === 'string') {
    cmd = {
      prefix: String(this.items.length + 1),
      text: item,
      callback: callback
    };
  }
  ...
  const _style = this.style.prefix || { fg: 'lightblack' };
  helpers.generateTags(_style, cmd.text);

  //title = (cmd.prefix != null ? t.open + cmd.prefix + t.close + ':' : '') + cmd.text;
  title = (cmd.prefix != null ? cmd.prefix + ':' : '') + cmd.text;
