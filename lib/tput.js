...
Tput.prototype._tprefix = function(prefix, term, soft) {
  ...
  if (!term) {
    ...
    try {
      ...
      // Prefix does not look like a terminfo directory.
    }
    return;
  }
...
Tput.prototype.detectUnicode = function() {
  if (process.env.NCURSES_FORCE_UNICODE != null) {
    return !!Number(process.env.NCURSES_FORCE_UNICODE);
  }
...
Tput.prototype.detectBrokenACS = function(info) {
  ...
  if (process.env.NCURSES_NO_UTF8_ACS != null) {
    return !!Number(process.env.NCURSES_NO_UTF8_ACS);
  }
...
Tput.prototype.GetConsoleCP = function() {
  ...
  try {
    ...
    // chcp may be unavailable depending on host environment.
  }
...
function sprintf(src) {
  ...
  return src.replace(rule, function(_, flag, width, type) {
    ...
    switch (type) {
      case 'd': // signed int
        param = (Number(param)).toString(10);
        break;
      case 'o': // unsigned octal
        param = (Number(param)).toString(8);
        break;
      case 'x': // unsigned hex int
        param = (Number(param)).toString(16);
        break;
      case 'X': // unsigned hex int uppercase
        param = (Number(param)).toString(16).toUppercase();
        break;
      case 's': // string
        break;
      case 'c': // char
